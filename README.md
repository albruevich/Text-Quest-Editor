...
Квест создается и хранится в формате JSON, что делает данную систему QuestEditor + QuestReader универсальной для того, чтобы можно было использовать свои квесты в любых своих проектах, и даже не обязательно основанных на Unity.
...
Нейтральная - никак не влияет на ход игры (если конечно не трогать влияние на параметры, но об этом потом)
...
Если же вы хотите ограничить, то снимите галку с чекбокса «неограниченная проходимость» и установите ограничение.
...
Есть 4 чекбокса: Единицы, Значение, Проценты, Формула
...
Если выбран чекбокс «Формула», то значение параметру устанавливается по формуле. Например в квесте «Астероидная станция» каждый переход убавлял кислород на 2 если пробоина не была устранена. p1 + (p11 == 0 ? -2 : 0)
