...
QuestEditor позволяет вам создавать и использовать текстовые квесты, наподобие квестов в в игре «Космические рейнджеры». 
QuestEditor представлен в виде готовых сборок для платформ Windows, maOS, Linux. 

Готовые квесты можно как проигрывать внутри самого QuestEditor в качестве тестирования, так и добавлять их внутрь QuestReader, который является опен-сорс, и дает возможность видеть и понимать работу квеста, использовать код QuestReader для своих проектов, настраивать UI вашего проекта на ваш вкус. 

Это два проекта (QuestEditor + QuestReader ) работают в связке. 

Квест создается и храниться в формате JSON, что делает данную систему QuestEditor + QuestReader универсальной для того, чтобы можно было использовать свои квесты в любых своих проектах, и даже не обязательно основанных на Unity.
...
1. нажмите на кнопку 
<img src="docs/btn_new.webp" width="92">
...
1. Нажмите на кнопку
<img src="docs/btn_load.webp" width="94">
...
1. Нажмите на кнопку
<img src="docs/btn_save.webp" width="88">
2. Нажмите на кнопку Save
3. Можно сохранять квест через горячие клавиши Cntrl + S (Windows), Cmd + S (macOS)
...
<img src="docs/quest_structure.webp" width="546">


Основная мысль / идея

Создание квеста основывается на трех ключевых понятиях:
Параметрах, локациях и переходах. 
1. Создаете параметры
2. Создаете локации
3. Создаете переходы между локациями
4. Каждая локация и переход может (но не обязательно) влиять на переход
5. Показ перехода также может зависеть от параметров
6. В описаниях и параметрах часто используются формулы, они могут быть таких типов: (p1+1)/2 и так далее
...
1. Для этого нажмите на кнопку
<img src="docs/btn_params.webp" width="84">
2. Нажмите на бокс слева от «Add parametr»
<img src="docs/box_add_param.webp" width="328">
3. Дайте ему Working title (это рабочее название, не для отображения)

4. Значения параметров

У каждого квеста есть минимальное, максимальное, и стартовое значение. Укажите их. 
Например, параметр Настроение. Оно может меняться от 1 до 3, а квест начинается со значением 2. 
Нажмите на кнопку Apply. 

<img src="docs/param_panel_1.webp" width="1782">
...
<img src="docs/param_types.webp" width="1784">
...
<img src="docs/param_disable.webp" width="318">
...
<img src="docs/quest_settings.webp" width="1786">
...
<img src="docs/modes.webp" width="354">
...
2. Если включен режим переходов, то кликните ЛКМ на локации «from», затем ЛКМ на локацию «to». После этого Появится панель создания нового перехода. (Кстати «from» и «to» могут быть одной и той же локацией). 
3. Если включен режим перемещения, то ЛКМ на локации перемещает локацию на пустую клетку, а ЛКМ на переходе дает возможность перемещать переходы между локациями. При этом имеет значение где был стартовый клик - на хвост или на голову перехода. 
...
<img src="docs/undo.webp" width="194">
...
<img src="docs/edit_location.webp" width="1790">
...
<img src="docs/loc_descr_1.webp" width="328">

Описания можно удалять. 

<img src="docs/loc_descr_2.webp" width="888">
...
Например формула: p1 > 5 ? 1 : 2 
означает что если p1 (то есть золото) больше 5 то будет показано описание 1, иначе 2

<img src="docs/loc_descr_3.webp" width="676">

Либо другой пример формулы: p2 + 1
Если пистолет есть, то покажется второе описание: p2 (оно равно 1, так как есть пистолет) + 1 = 2 (второе описание)
Если пистолета нет, то покажется первое описание: p2 (оно равно 0, так как пистолета нет) + 1 = 1 (первое описание)

<img src="docs/loc_descr_4.webp" width="1414">

Сами описания локаций могут принимать значения параметров. 
Например. «Вы сегодня так устали, Ваше зродоввье где-то на {p3}». 
В игре же будет показано:  «Вы сегодня так устали, Ваше зродоввье где-то на 7»

2. Типы локаций

Локации могут быть разных типов: Нейтральные, Стартовая, Победная, Проваленная, Пустая. 

<img src="docs/loc_types.webp" width="346">

Нейтральная - никак не влияет на ход игры (если конечно не трогать влияние на параметры, но оба этом потом)
Стартовая - именно с нее начинается квест. Может только одна стартовая локация. 
Победная - тут все понятно, на ней завершается победное прохождение квеста
Проваленная - аналогично с победным, только все грустно.
...
По умолчанию локация создается с неограниченной проходимостью (игрок может попадать в нее бесконечное количество раз) 

<img src="docs/loc_pass_1.webp" width="368">

Если же вы хотите ограничить, то снимите галку с чекбокса «неограниченная прохидомость» и установите ограничение.
Например пришел к другу 3 раза, а на 4-й раз замок висит на двери, и надпись: «Надоел ты мне своей назойливостью»

<img src="docs/loc_pass_2.webp" width="398">

Также это очень удобно чтобы делать локации одноразовыми. 

4. Влияние локации на параметры
Когда игрок попадает в локацию, она может изменить параметр
Для этого нужно выбрать параметр. 

<img src="docs/loc_infl_to_param.webp" width="1752">

Действия: 
локация может показать параметр, и в таком случае он будет показан в игре в окне параметров. 
Локация может скрыть параметр. 
Либо ничего не делать (не показывать и не прятать). Этот флаг стоит по умолчанию. 

Влияние на знания параметров:
Есть 4 чекбокса: Единицы, Значение, Проценнты, Формула

Если выбран чекбокс «Единицы», то локация может прибавить либо убавить. Например было 2 золота, локация указывает +3. При подадании в эту локацию золота станет 5 (но не больше чем максимальное и минимальное значение параметра)

Если выбран чекбокс «Значение», то устанавливается ровно такое значение, какое указано в локации. Например было 2 золота, а локация устанавливает 0. Больше золота у игрока нет. 

Если выбран чекбокс «Проценты», то локация прибавляет или отнимает относительное значение от параметра. Например, было 10 золота, локация отнимает 50%. Теперь у игрока стало 5 золота. 

Если выбран чекбокс «Формула», то значение параметру устанавливается по формуле. Например в квеста «Астероидная станция» каждый переход убавлял кислород на 2 если пробоина не была устранена. p1 + (p11 == 0 ? -2 : 0)
Здесь p1 — кислород, p11 — пробоина залатана. 
Формулы могут быть самыми разными. Например выбрать параметр p1 — золото, и в формуле установит (p2 + p5) / 2. То есть, при попадании в локацию золота у игрока станет: количество часов работы + премия и все это поделить на 2 

Рандом. 
Все формулы могут принимать рандом. Например в квеста «астероидная станция» пистолетный выстрел наносит монстру урон:
p9 - rnd(1, 3)
...
<img src="docs/trans_range.webp" width="478">
...
В зтом примере переход будет показан только в том случае, если выбранный параметр p1 (золото) принимает значения: 1 или яблоки минус 2, или случайное число от 3 до 10

<img src="docs/trans_accepts.webp" width="482">
...
<img src="docs/trans_div.webp" width="478">
...
<img src="docs/trans_order_1.webp" width="548">
...
<img src="docs/trans_order_2.webp" width="254">
...
<img src="docs/trans_prior_1.webp" width="1102">
...
<img src="docs/trans_prior_2.webp" width="438">
...
<img src="docs/trans_always_show_1.webp" width="780">

Например недоступно пройти в Медблок или в аварийный отсек (так как монстр закрывает дорог, условия показа перехода не выполнены). 
...
<img src="docs/trans_always_show_2.webp" width="274">
...
<img src="docs/res_folders.webp" width="716">
...
<img src="docs/res_tags_1.webp" width="1770">

<img src="docs/res_tags_2.webp" width="1786">
...
<img src="docs/res_cache.webp" width="450">
...
<img src="docs/btn_play.webp" width="108">
...
Если включены подсказки, то также при наведении на элементы квеста (локации и переходы), будут появляться тултипс с инфой про элементы квеста

Можете выключить / включить сетку 
<img src="docs/grid_n_hints.webp" width="75">

Вы также можете посмотреть короткое инфо об управлении Редактором через мышь
<img src="docs/btn_info.webp" width="52">

Хвост перехода (from) синего цвета и широкий, голова перехода (to) белого цвета и узкая
<img src="docs/trans_view.webp" width="345">


Советы

Не беритесь сразу за сложный квест. Для начала напишите небольшой квест из 5 - 10 локаций. Т.е. учитесь пользоваться редактором потихоньку. При создании маленького квеста вы получите необходимый навык и уже не повторите старых ошибок, когда будите делать что-то более сложное.
