...
- использовать параметры, локации, переходы и формулы.
...
- [Локальные и удалённые квесты](#локальные-и-удалённые-квесты)
- [Локальное сохранение квеста](#локальное-сохранение-квеста)
- [Примеры квестов](#примеры-квестов)
- [Добавление квеста в Text Quest Reader](#добавление-квеста-в-text-quest-reader)
...
- квесты, опубликованные на сервере,
...
В редакторе доступны два источника квестов:

- **Local** — квесты, сохранённые на вашем компьютере.
- **Remote** — квесты, загруженные с сервера.

В панели загрузки можно переключаться между локальными и удалёнными квестами.

Если сервер недоступен, вкладка **Remote** будет отключена.

Квесты, загруженные с сервера, имеют уникальный ID, который отображается справа от названия.

Например: `#1`

Этот ID используется для идентификации квеста на сервере.
...
Для публикации квестов на сервере необходимо авторизоваться.

После авторизации:
- новые квесты можно публиковать удалённо,
- уже опубликованные квесты можно обновлять,
- ваши квесты будут помечены как принадлежащие вам.

---

## Авторизация

Чтобы открыть панель авторизации:
...
## Локальное сохранение квеста

Квесты можно сохранять локально на компьютер, включая квесты, загруженные с сервера.
...
4. Нажмите кнопку **Refresh** или перезапустите приложение — квест появится в списке доступных
...
При локальном использовании название папки не должно изменяться.
...
**Language** определяет язык системных элементов квеста в [**Text Quest Reader**](https://github.com/albruevich/Text-Quest-Reader):

- текст кнопки **Next**,
- сообщения победы и поражения,
- языковую метку квеста в списке, например `[EN]`.
